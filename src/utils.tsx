...
export const sortByLevenshteinDistance = (options: SelectChildType[], baseString: string) => {
    if (options.length === 0) return options;

    let result;
    if (typeof options[0] === "string") {
        result = (options as string[]).slice()
            .sort((a, b) => compareByLevenshteinDistance(a, b, baseString));

    } else {
        result = (options as ReactElement[]).slice()
            .sort((a, b) => compareByLevenshteinDistanceReactElement(a, b, baseString));
    }
...
export const getIdxLowestLevenshteinDistance = (options: SelectChildType[], baseString: string) => {
    if (options.length === 0) return -1;

    if (typeof options[0] === "string") {
        return getLowestLevenshteinDistanceByString(options as string[], baseString);
    }
...
function getLowestLevenshteinDistanceByString(arr : string[], baseString: string) {
    let lowestDist: number = editDistance.levenshtein(baseString, arr[0], insert, remove, update).distance;
    let lowIdx = 0;

    for (let i = 1; i < arr.length; i++) {
        const currentDist: number = editDistance.levenshtein(baseString, arr[i], insert, remove, update).distance;
...
function compareByLevenshteinDistanceReactElement(a: ReactElement, b: ReactElement, baseString: string) {
    return compareByLevenshteinDistance(extractStrFromElement(a), extractStrFromElement(b), baseString);
}

function compareByLevenshteinDistance(a: string, b: string, baseString: string) {
    if (baseString === "") {
        return 0;
    }
    ...
    if (elementToLevDistanceMap.has(a)) {
        ...
        distanceA = editDistance.levenshtein(baseString, a, insert, remove, update).distance;
    ...
    if (elementToLevDistanceMap.has(b)) {
        ...
        distanceB = editDistance.levenshtein(baseString, b, insert, remove, update).distance;
